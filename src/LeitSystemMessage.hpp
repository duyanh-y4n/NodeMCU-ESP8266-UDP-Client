namespace {
    namespace Message{
        ...
        // Message utils
        bool isRegisterResponseFromServer(char* response){ //only first byte is different from request header
            return response[REQUEST_ID_BYTE_POSITION] == REGISTER_REQ;
        }

        bool isDrivePermissionResponseFromServer(char* response){ //only first byte is different from request header
            return response[REQUEST_ID_BYTE_POSITION] == CAR_STATE_REQ;
        }
    }
}